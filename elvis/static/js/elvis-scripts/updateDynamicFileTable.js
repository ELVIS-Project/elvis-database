...
(function ($)
{
    ...
    var dynamicTable = function(element, options)
    {
        ...
        function addRow(exists)
        {
            ...
            if (settings.movement)
            {
                ...
                settings.$table.append("<tr id='" + e + settings.table_name + settings.row_count + "'>" +
                    ...
                    "<td class='text-center'><input class='form-control' style='text-align:center; min-width: 50px'id='" + settings.table_name + settings.row_count + "_position' name='" + settings.table_name + settings.row_count + "_position' ></td>" +
                ...
                $("#" + settings.table_name + settings.row_count + '_position').val(settings.t_size);
            }
            else
            {
        ...
        function initRow(row, exists)
        {
            ...
            {
                if (settings.table_name === "mov")
                {
                    var row = $(this).parent().parent();
                    var renumberFrom = settings.$table.children().index(row) + 2;
                    renumberRows(renumberFrom);
                }
        ...
        function renumberRows(renumberFrom)
        {
            var $tableChildren = settings.$table.children();
            for (var j = renumberFrom; j < $tableChildren.length; j += 2)
            {
                $tableChildren[j].children[1].children[0].value = (j-1)/2;
            }
        }
